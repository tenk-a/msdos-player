...
//	mem[DUMMY_TOP + 0x08] = 0x45;
	...
	*(UINT16 *)(mem + FCB_TABLE_TOP + 4) = 0;
	...
	//æš«å®šå¯¾å‡¦: ãƒ‰ãƒ©ã‚¤ãƒ–ãƒ¬ã‚¿ãƒ¼æœ‰ã‚Šã ãŒå­˜åœ¨ã—ãªã„ãƒãƒƒãƒˆãƒ¯ãƒ¼ã‚¯ãƒ‰ãƒ©ã‚¤ãƒ–ã®ãƒã‚§ãƒƒã‚¯ã§
	// GetDriveTypeA ãŒç•°æ§˜ã«æ™‚é–“ãŒã‹ã‹ã‚‹ã“ã¨ãŒã‚ã£ãŸã€‚
	// å°‘ãªãã¨ã‚‚èµ·å‹•æ™‚ã¯ LAST_DRIVE ä»¥é™ã®(ãƒãƒƒãƒˆãƒ¯ãƒ¼ã‚¯)ãƒ‰ãƒ©ã‚¤ãƒ–ã®ãƒã‚§ãƒƒã‚¯ã‚’è¡Œã‚ãªã„ã‚ˆã†ã«å¯¾å‡¦.
